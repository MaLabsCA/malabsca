...
class WooExporter(AbstractComponent):
    ...
    def run(self, odoo_record, force=False):
        ...
        ups_id = self._get_ups_id()
        if not force and self._is_uptodate(ups_id):
        ...
        if ups_id:
            ...
            self._update(ups_id, record)
        else:
            record = self._create_data(map_record)
            malabs_id = self.backend_adapter.create(record).get('id')
            if malabs_id:
                self.binder.bind(malabs_id, self._create(record))

        self._after_export(malabs_id)
...
class BatchExporter(AbstractComponent):
    ...
    def run(self, filters=None):
        ...
        if not filters:
            filters = []
        records = self.model.odoo_id.search(filters)
        _logger.info('search for %s with %s returned %s', self.model.odoo_id._name, filters, records.ids)
...
class DelayedBatchExporter(AbstractComponent):
    ...
    _inherit = 'ups.batch.exporter'
    _name = 'ups.delayed.batch.exporter'
